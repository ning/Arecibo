...
// For each of the following tt_config variables there exists a command, which is
...
tt_config. BgColor			= '#E2E7FF'	// Background colour (HTML colour value, in quotes)
tt_config. BgImg			= ''		// Path to background image, none if empty string ''
tt_config. BorderColor		= '#003099'
...
tt_config. ClickSticky		= false		// false or true - make tooltip sticky if user left-clicks on the hovered element while the tooltip is active
tt_config. CloseBtn		= false		// false or true - closebutton in titlebar
tt_config. CloseBtnColors	= ['#990000', '#FFFFFF', '#DD3333', '#FFFFFF']	// [Background, text, hovered background, hovered text] - use empty strings '' to inherit title colours
...
tt_config. Duration		= 0			// Time span in ms after which the tooltip disappears; 0 for infinite duration, < 0 for delay in ms _after_ the onmouseout until the tooltip disappears
tt_config. Exclusive		= false		// false or true - no other tooltip can appear until the current one has actively been closed
tt_config. FadeIn			= 100		// Fade-in duration in ms, e.g. 400; 0 for no animation
tt_config. FadeOut			= 100
tt_config. FadeInterval	= 30		// Duration of each fade step in ms (recommended: 30) - shorter is smoother but causes more CPU-load
tt_config. Fix				= null		// Fixated position, two modes. Mode 1: x- an y-coordinates in brackets, e.g. [210, 480]. Mode 2: Show tooltip at a position related to an HTML element: [ID of HTML element, x-offset, y-offset from HTML element], e.g. ['SomeID', 10, 30]. Value null (default) for no fixated positioning.
...
tt_config. Height			= 0			// Tooltip height; 0 for automatic adaption to tooltip content, < 0 (e.g. -100) for a maximum for automatic adaption
tt_config. JumpHorz		= false		// false or true - jump horizontally to other side of mouse if tooltip would extend past clientarea boundary
tt_config. JumpVert		= true		// false or true - jump vertically		"
...
tt_config. Sticky			= false		// false or true - fixate tip, ie. don't follow the mouse and don't hide on mouseout
...
tt_config. TitlePadding	= 2
tt_config. Width			= 0			// Tooltip width; 0 for automatic adaption to tooltip content; < -1 (e.g. -240) for a maximum width for that automatic adaption;
