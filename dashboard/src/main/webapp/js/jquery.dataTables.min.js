...
var y=b.replace(/<.*?>/g,"").toLowerCase();return((x<y)?-1:((x>y)?1:0))},"date-asc":function(a,b){return Date.parse(a)-Date.parse(b)
},"date-desc":function(a,b){return Date.parse(a)+Date.parse(b)},"numeric-asc":function(a,b){var x=a=="-"?0:a;
var y=b=="-"?0:b;return x-y},"numeric-desc":function(a,b){var x=a=="-"?0:a;var y=b=="-"?0:b;
return x+y}};$.fn.dataTable=function(oInit){var _aoSettings=$.fn.dataTableSettings;
...
this.iDisplayEnd=10;this.aaSorting=[[0,"asc"]];this.asStripClasses=new Array();this.fnRowCallback=null;
this.fnHeaderCallback=null;this.fnFooterCallback=null;this.nFooter=null;this.sTableId="";
this.nTable=null;this.iDefaultSortIndex=0;this.bInitialised=false;this.nOpenRow=null;
...
}var bUserSetType=oSettings.aoColumns[i].sType==null?false:true;$("tbody td:nth-child("+oSettings.aoColumns.length+"n+"+(i+1)+")",oSettings.nTable).each(function(index){if(typeof oSettings.aaData[index]!="object"){oSettings.aaData[index]=new Array()
...
}if(oSettings.oFeatures.bProcessing){_fnProcessingDisplay(oSettings,true)}if(e.shiftKey){var bFound=false;
...
};return false})}oSettings.nTable.style.width=oSettings.nTable.offsetWidth+"px"}function _fnDraw(oSettings){var anRows=new Array();
var sOutput="";var iRowCount=0;var nTd;var i;if(oSettings.aaData.length!=0){for(var j=oSettings.iDisplayStart;
j<oSettings.iDisplayEnd;j++){anRows[iRowCount]=document.createElement("tr");if(oSettings.asStripClasses.length>0){anRows[iRowCount].className=oSettings.asStripClasses[iRowCount%oSettings.asStripClasses.length]
...
j++){if(oSettings.aoColumns[j].bSearchable){oSettings.asDataSearch[i]+=aArray[i][j].replace(/\n/g," ")+" "
}}}}function _fnCalculateEnd(oSettings){if(oSettings.oFeatures.bPaginate==false){oSettings.iDisplayEnd=oSettings.aaData.length
...
}else{oSettings.asStripClasses=["odd","even"]}if(typeof oInit.fnRowCallback!="undefined"){oSettings.fnRowCallback=oInit.fnRowCallback
...
for(var i=0;i<oSettings.aoColumns.length;i++){oSettings.aoColumns[i].sType="string"
