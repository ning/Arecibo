 ...
 * <p>
 * The Map<Integer, Map<Integer, DateTime>> maps from hostId to eventCategoryId to startTime.
 */
public class StartTimes {
    ...
    public StartTimes(DateTime timeInserted, Map<Integer, Map<Integer, DateTime>> startTimesMap)
    {
    ...
    public void addTime(final int hostId, final int categoryId, final DateTime dateTime)
    {
        Map<Integer, DateTime> hostTimes = startTimesMap.get(hostId);
        if (hostTimes == null) {
            hostTimes = new HashMap<Integer, DateTime>();
            startTimesMap.put(hostId, hostTimes);
        }
        hostTimes.put(categoryId, dateTime);
        if (dateTime.isBefore(minStartTime)) {
            minStartTime = dateTime;
        }
    }

    public DateTime getStartTimeForHostIdAndCategoryId(final int hostId, final int categoryId)
    {
        final Map<Integer, DateTime> hostTimes = startTimesMap.get(hostId);
        if (hostTimes != null) {
            return hostTimes.get(categoryId);
        }
        else {
            return null;
        }
